...
rule kraken2:
    ...
    output:
        ...
        classified_r1 = "results/kraken2/{sample}_classified_1.fastq.gz",
        classified_r2 = "results/kraken2/{sample}_classified_2.fastq.gz",
        unclassified_r1 = "results/kraken2/{sample}_unclassified_1.fastq.gz",
        unclassified_r2 = "results/kraken2/{sample}_unclassified_2.fastq.gz"
    threads: 16
    resources:
        mem_mb = 64000
    log:
    ...
    shell:
        ...
        --use-names \
        --classified-out results/kraken2/{wildcards.sample}_classified#.fastq \
        --unclassified-out results/kraken2/{wildcards.sample}_unclassified#.fastq \
        2> {log}
        
        # Compress the output fastq files
        gzip results/kraken2/{wildcards.sample}_classified_1.fastq
        gzip results/kraken2/{wildcards.sample}_classified_2.fastq
        gzip results/kraken2/{wildcards.sample}_unclassified_1.fastq
        gzip results/kraken2/{wildcards.sample}_unclassified_2.fastq
        """

# Metagenomic assembly with metaSPAdes
rule metaspades:
    input:
        r1 = lambda wildcards: "results/kraken2/{}_unclassified_1.fastq.gz".format(wildcards.sample) if config["use_unclassified_only"] else "results/preprocessing/{}_host_removed_1.fastq.gz".format(wildcards.sample),
        r2 = lambda wildcards: "results/kraken2/{}_unclassified_2.fastq.gz".format(wildcards.sample) if config["use_unclassified_only"] else "results/preprocessing/{}_host_removed_2.fastq.gz".format(wildcards.sample)
    output:
